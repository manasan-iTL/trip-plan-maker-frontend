...
const TopContainer = () => {
    ...
    const [prefectures, setPrefectures] = useState<prefectureState[]>(prefecturesData)
    ...
    const handleChangePrefectureCheckbox = (value: string) =>{
        setPrefectures(prevState => {
            const newPrefectures = prevState.map(prefecture => { 

                // 子要素が変わったかの判定フラグ
                let isChangeValue = false

                const newChildItems = prefecture.childItems.map(childItem => {
                    if (childItem.value === value) {
                        isChangeValue = true
                        const newChildItem = { ...childItem, checked: !childItem.checked }
                        return newChildItem
                    }
                ...
                if (!isChangeValue) return  prefecture
                const newPrefecture = { ...prefecture, childItems: newChildItems }
                isChangeValue = false
    ...
    const handleClickNextPage = async () => {
        ...
        try {
            const result2 = await postFetcher<SearchSpotsRequestBody, SearchSpotsResponseBody>(BASE_URL + "/api/v2/spots", newBody)
            console.log(result2)
    ...
    return (
        <TopTemplate 
            ...
            handleChangePrefecureCheckbox={handleChangePrefectureCheckbox}
            tripDate={tripDate}
